...
Before we break, point out that we can chain things together. For example, earlier we used
`data_oceania_country.T` to transpose the data, then we used `data_oceania_country.describe()` to
get summary statistics. We could have done this in a single line of code by 'chaining' the commands:
